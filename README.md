...
mining-agent-mcp/
├── README.md
├── mcp-config.json                  # 可直接接入 Claude Desktop / Cursor
├── Dockerfile                       # Python 3.10-slim 镜像
├── docker-compose.yml               # 一键启动
├── requirements.txt
├── src/
│   ├── agent/
│   │   └── client.py                # LangGraph ReAct Agent（大脑）
│   └── servers/
│       ├── mining_news/server.py    # DuckDuckGo 新闻搜索
│       ├── mineral_pdf/server.py    # NI 43-101 PDF 储量提取
│       └── lme_price/server.py      # yfinance 价格查询
```
...
> **💡 Mock 降级验证模式**
> 
> 如果未配置 API Key，系统不会报错崩溃，而是优雅进入 Mock 模式：
> 自动通过 `stdio` 拉起全部 MCP 子进程 → 拉取并验证工具 JSON Schema → 打印成功日志后退出。
> 面试官即使在没有 Key 的环境下，也能验证底层 MCP 架构的完整连通性。
...
> **提示**：此模式下无需为本项目配置任何 API Key，Claude 自身作为推理引擎，3 个 MCP Server 只负责数据抓取，完全免费运行。
